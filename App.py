...
def run() -> object:
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'sgu@123' and ad_password == '123':
                st.success("Welcome Shardul")
                # Display Data
                ...
                fig = px.pie(plot_data, values=plot_data['Predicted_Field'].value_counts().values, names=plot_data['Predicted_Field'].unique(), title='Predicted Field according to the Skills')
                st.plotly_chart(fig)
                ...
                fig = px.pie(plot_data, values=plot_data['User_level'].value_counts().values, names=plot_data['User_level'].unique(), title="Pie-Chart📈 for User's👨‍💻 Experienced Level")
                st.plotly_chart(fig)
